...
- crates.io package: `gpui-pre-macos` 0.3.4
- declared upstream repository: https://github.com/zed-industries/zed
- Zed snapshot recorded by the crate: `69164008341295ad481bb11c0334a712ca8c23e3`
- local delta: `patches/gpui-pre-macos-embedded-runloop.patch`

The patch touches only `src/platform.rs`. It adds an embedded macOS event-loop mode so GPUI can run inside a host process (Retend's Node/napi binding) instead of taking over the process with a blocking `CFRunLoopRun()`.
