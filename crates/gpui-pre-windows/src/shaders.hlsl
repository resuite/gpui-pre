...
// Sparse 2D transform state. The first uint of every instance record carries
// the one-based spatial id (0 is identity); the CPU-only draw order is not
// uploaded. Spatial states live in t2, ancestor clips in t3. Coefficients are
// stored row-major as (m00, m01, m10, m11) and applied with explicit dots so
// the result matches the Rust `TransformationMatrix::apply` exactly.
struct SpatialState {
    float4 matrix_coeffs;
    float2 translation;
    uint clip_start;
    uint clip_count;
};
...
struct TransformedClip {
    float4 inverse_coeffs;
    float2 inverse_translation;
    float2 bounds_origin;
    float2 bounds_size;
};

StructuredBuffer<SpatialState> spatial_states: register(t2);
StructuredBuffer<TransformedClip> transform_clips: register(t3);

float2 apply_spatial_state(float2 position, SpatialState state) {
    return float2(dot(position, state.matrix_coeffs.xy),
                  dot(position, state.matrix_coeffs.zw))
        + state.translation;
}

float2 apply_inverse_clip(float2 position, TransformedClip clip) {
    return float2(dot(position, clip.inverse_coeffs.xy),
                  dot(position, clip.inverse_coeffs.zw))
        + clip.inverse_translation;
}

float4 transform_device_position(float4 position, SpatialState state) {
    float2 local = (position.xy - float2(-1.0, 1.0)) / float2(2.0, -2.0)
        * global_viewport_size;
    float2 transformed = apply_spatial_state(local, state);
    float2 device = transformed / global_viewport_size * float2(2.0, -2.0)
        + float2(-1.0, 1.0);
    return float4(device, position.z, position.w);
}
...
float4 quad_shade_impl(Quad quad, float2 local_position, float4 border_color,
                         float4 background_solid, float4 background_color0,
...
float4 quad_fragment(QuadFragmentInput input): SV_Target {
    ...
    return quad_shade_impl(quad, input.position.xy, input.border_color,
...
float4 shadow_shade_impl(Shadow shadow, float2 local_position, float4 color) {
...
float4 shadow_fragment(ShadowFragmentInput input): SV_TARGET {
    ...
    return shadow_shade_impl(shadow, input.position.xy, input.color);
}
...
float4 underline_shade_impl(Underline underline, float2 local_position, float4 color) {
...
float4 underline_fragment(UnderlineFragmentInput input): SV_Target {
    ...
    return underline_shade_impl(underline, input.position.xy, input.color);
}
...
float4 polychrome_shade_impl(PolychromeSprite sprite, float2 local_position, float4 sample) {
...
float4 polychrome_sprite_fragment(PolychromeSpriteFragmentInput input): SV_Target {
    ...
    return polychrome_shade_impl(sprite, input.position.xy, sample);
}


/*
**
**              Sparse transformed primitives
**
** The first uint of every instance record carries the one-based spatial id
** (0 is identity and always uses the ordinary entry points above).
*/
...
QuadTransformedVertexOutput quad_transformed_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    ...
    uint quad_id = batch_start_index + instance_id;
    Quad quad = quads[quad_id];
    SpatialState spatial = spatial_states[quad.order - 1u];

    float4 device_position = transform_device_position(
        to_device_position(unit_vertex, quad.bounds), spatial);
    GradientColor gradient = prepare_gradient_color(
        quad.background.tag,
        quad.background.color_space,
        quad.background.solid,
...
ShadowTransformedVertexOutput shadow_transformed_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    uint shadow_id = batch_start_index + instance_id;
    Shadow shadow = shadows[shadow_id];
    SpatialState spatial = spatial_states[shadow.order - 1u];
    ...
    float2 local_position = bounds.origin + unit_vertex * bounds.size;
    ShadowTransformedVertexOutput output;
    output.position = transform_device_position(
        to_device_position(unit_vertex, bounds), spatial);
    output.color = hsla_to_rgba(shadow.color);
    output.shadow_id = shadow_id;
    output.clip_distance = distance_from_clip_rect(unit_vertex, bounds, shadow.content_mask);
...
UnderlineTransformedVertexOutput underline_transformed_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    uint underline_id = batch_start_index + instance_id;
    Underline underline = underlines[underline_id];
    SpatialState spatial = spatial_states[underline.order - 1u];

    UnderlineTransformedVertexOutput output;
    output.position = transform_device_position(
        to_device_position(unit_vertex, underline.bounds), spatial);
    output.color = hsla_to_rgba(underline.color);
    output.underline_id = underline_id;
    output.clip_distance = distance_from_clip_rect(unit_vertex, underline.bounds,
        underline.content_mask);
    output.local_position = underline.bounds.origin + unit_vertex * underline.bounds.size;
    output.clip_range = uint2(spatial.clip_start, spatial.clip_count);
    return output;
}
...
struct MonochromeSpriteTransformedVertexOutput {
    ...
    nointerpolation float4 color: COLOR;
    float4 clip_distance: SV_ClipDistance;
    nointerpolation uint2 clip_range: TEXCOORD0;
};
...
MonochromeSpriteTransformedVertexOutput monochrome_sprite_transformed_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    uint sprite_id = batch_start_index + instance_id;
    MonochromeSprite sprite = mono_sprites[sprite_id];
    SpatialState spatial = spatial_states[sprite.order - 1u];

    float2 tile_position = to_tile_position(unit_vertex, sprite.tile);
    MonochromeSpriteTransformedVertexOutput output;
    output.position = transform_device_position(
        to_device_position_transformed(unit_vertex, sprite.bounds, sprite.transformation),
        spatial);
    output.tile_position = tile_position;
    output.color = hsla_to_rgba(sprite.color);
    output.clip_distance = distance_from_clip_rect_transformed(unit_vertex, sprite.bounds,
...
PolychromeSpriteTransformedVertexOutput polychrome_sprite_transformed_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    uint sprite_id = batch_start_index + instance_id;
    PolychromeSprite sprite = poly_sprites[sprite_id];
    SpatialState spatial = spatial_states[sprite.order - 1u];

    PolychromeSpriteTransformedVertexOutput output;
    output.position = transform_device_position(
        to_device_position(unit_vertex, sprite.bounds), spatial);
...
float4 polychrome_sprite_transformed_fragment(PolychromeSpriteTransformedFragmentInput input): SV_Target {
    ...
    PolychromeSprite sprite = poly_sprites[input.sprite_id];
    float4 sample = t_sprite.Sample(s_sprite, input.tile_position);
    return polychrome_shade_impl(sprite, input.local_position, sample);
}
