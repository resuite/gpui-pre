...
pub trait MouseEvent: InputEvent + Clone {
    /// Map pointer coordinates into an element's layout space.
...
impl MouseEvent for MouseDownEvent {
    fn transformed(&self, matrix: crate::TransformationMatrix) -> Self {
        ...
        event.position = matrix.apply(event.position);
        event
    }
}
...
impl MouseEvent for FileDropEvent {
    fn transformed(&self, matrix: crate::TransformationMatrix) -> Self {
        ...
        match &mut event {
            ...
            | Self::Submit { position } => *position = matrix.apply(*position),
            Self::Exited | Self::Ended => {}
        }
        event
    }
}
