...
use crate::gpu_types::{
    GpuMonochromeSprite, GpuPolychromeSprite, GpuQuad, GpuShadow, GpuSpatialState,
    GpuTransformedClip, GpuUnderline,
};
use crate::*;
use gpui::*;
...
struct DirectXRenderPipelines {
    shadow_pipeline: PipelineState<GpuShadow>,
    quad_pipeline: PipelineState<GpuQuad>,
    path_rasterization_pipeline: PipelineState<PathRasterizationSprite>,
    path_sprite_pipeline: PipelineState<PathSprite>,
    underline_pipeline: PipelineState<GpuUnderline>,
    mono_sprites: PipelineState<GpuMonochromeSprite>,
    subpixel_sprites: PipelineState<GpuMonochromeSprite>,
    poly_sprites: PipelineState<GpuPolychromeSprite>,
}

/// Uploaded sparse transform state. Identity primitives (`spatial_id == 0`)
/// never read these buffers; transformed draws bind them at `t2`/`t3`.
struct SpatialBuffers {
    states: Option<ID3D11Buffer>,
    states_view: Option<ID3D11ShaderResourceView>,
    states_capacity: usize,
    clips: Option<ID3D11Buffer>,
    clips_view: Option<ID3D11ShaderResourceView>,
    clips_capacity: usize,
}
...
impl DirectXRenderer {
    ...
    fn handle_device_lost_impl(&mut self, directx_devices: &DirectXDevices) -> Result<()> {
        ...
        // Spatial buffers hold device objects; they are re-uploaded on the
        // next transformed frame.
        self.spatial.clear();
        ...
        for batch in scene.batches() {
            ...
            match batch {
                PrimitiveBatch::Shadows(range) => {
                    ...
                    self.draw_shadows(range.start, range.len(), transformed)
                }
                PrimitiveBatch::Quads(range) => {
                    let transformed = !scene.quads[range.start].spatial_id.is_identity();
                    self.draw_quads(range.start, range.len(), transformed)
                }
                PrimitiveBatch::Paths(range) => {
                    let paths = &scene.paths[range];
                    self.draw_paths_to_intermediate(scene, paths)?;
                    self.draw_paths_from_intermediate(scene, paths)
                }
                PrimitiveBatch::Underlines(range) => {
                    let transformed = !scene.underlines[range.start].spatial_id.is_identity();
                    self.draw_underlines(range.start, range.len(), transformed)
                }
                PrimitiveBatch::MonochromeSprites { texture_id, range } => {
                    let transformed = !scene.monochrome_sprites[range.start]
                        .spatial_id
                        .is_identity();
                    self.draw_monochrome_sprites(texture_id, range.start, range.len(), transformed)
                }
                PrimitiveBatch::SubpixelSprites { texture_id, range } => {
                    let transformed = !scene.subpixel_sprites[range.start]
                        .spatial_id
                        .is_identity();
                    self.draw_subpixel_sprites(texture_id, range.start, range.len(), transformed)
                }
                PrimitiveBatch::PolychromeSprites { texture_id, range } => {
                    let transformed = !scene.polychrome_sprites[range.start]
                        .spatial_id
                        .is_identity();
                    self.draw_polychrome_sprites(texture_id, range.start, range.len(), transformed)
                }
    ...
    fn upload_scene_buffers(&mut self, scene: &Scene) -> Result<()> {
        ...
        // GPU records replace the CPU-only draw order with the spatial id,
        // keeping the upstream instance strides the HLSL structs expect.
        if !scene.shadows.is_empty() {
            let shadows: Vec<GpuShadow> = scene.shadows.iter().map(GpuShadow::from).collect();
        ...
        if !scene.quads.is_empty() {
            let quads: Vec<GpuQuad> = scene.quads.iter().map(GpuQuad::from).collect();
        ...
        if !scene.underlines.is_empty() {
            let underlines: Vec<GpuUnderline> =
                scene.underlines.iter().map(GpuUnderline::from).collect();
        ...
        if !scene.monochrome_sprites.is_empty() {
            let sprites: Vec<GpuMonochromeSprite> = scene
                .monochrome_sprites
                .iter()
        ...
        if !scene.subpixel_sprites.is_empty() {
            let sprites: Vec<GpuMonochromeSprite> = scene
                .subpixel_sprites
                .iter()
                .map(|sprite| GpuMonochromeSprite {
                    spatial_id: sprite.spatial_id.0,
                    pad: sprite.pad,
        ...
        if !scene.polychrome_sprites.is_empty() {
            let sprites: Vec<GpuPolychromeSprite> = scene
                .polychrome_sprites
                .iter()
        ...
        self.upload_spatial_buffers(scene)?;

        Ok(())
    }

    fn upload_spatial_buffers(&mut self, scene: &Scene) -> Result<()> {
        let devices = self.devices.as_ref().context("devices missing")?;
        if scene.spatial_states.is_empty() {
            return Ok(());
        }
        let states: Vec<GpuSpatialState> = scene
            .spatial_states
            .iter()
            .map(GpuSpatialState::from)
            .collect();
        ensure_spatial_buffer(
            &devices.device,
            &mut self.spatial.states,
            &mut self.spatial.states_view,
            &mut self.spatial.states_capacity,
            &states,
        )?;
        update_buffer(
            &devices.device_context,
            self.spatial.states.as_ref().unwrap(),
            &states,
        )?;
        if !scene.transform_clips.is_empty() {
            let clips: Vec<GpuTransformedClip> = scene
                .transform_clips
                .iter()
                .map(GpuTransformedClip::from)
                .collect();
            ensure_spatial_buffer(
                &devices.device,
                &mut self.spatial.clips,
                &mut self.spatial.clips_view,
                &mut self.spatial.clips_capacity,
                &clips,
            )?;
            update_buffer(
                &devices.device_context,
                self.spatial.clips.as_ref().unwrap(),
                &clips,
            )?;
        } else {
            // Transformed shaders always declare the clip buffer. Bind a
            // single dummy entry so `t3` is valid even when no ancestor
            // clips exist; `clip_count == 0` means it is never read.
            let dummy = [GpuTransformedClip {
    ...
    fn draw_shadows(&mut self, start: usize, len: usize, transformed: bool) -> Result<()> {
        if len == 0 {
            return Ok(());
        }
        let devices = self.devices.as_ref().context("devices missing")?;
        ...
        if transformed {
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed shadows without uploaded spatial state");
            };
            self.pipelines.shadow_pipeline.draw_range_transformed(
                &devices.device_context,
                batch_params,
                states,
                clips,
    ...
    fn draw_quads(&mut self, start: usize, len: usize, transformed: bool) -> Result<()> {
        if len == 0 {
            return Ok(());
        }
        let devices = self.devices.as_ref().context("devices missing")?;
        ...
        if transformed {
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed quads without uploaded spatial state");
            };
            self.pipelines.quad_pipeline.draw_range_transformed(
                &devices.device_context,
                batch_params,
                states,
                clips,
    ...
    fn draw_paths_to_intermediate(
        ...
        // Transformed paths are baked on the CPU: the intermediate texture
        // lives in screen space, so pre-transforming the vertices lets the
        // ordinary rasterization pipeline draw them without new shaders.
        // Ancestor cross-space clips are not applied to paths here.
        let mut vertices = Vec::new();

        for path in paths {
            let matrix = scene.spatial_matrix(path.spatial_id);
            let transformed = !path.spatial_id.is_identity();
            vertices.extend(path.vertices.iter().map(|v| {
                let xy_position = if transformed {
                    apply_scaled_matrix(&matrix, v.xy_position)
                } else {
                    v.xy_position
                };
                PathRasterizationSprite {
                    xy_position,
                    st_position: v.st_position,
                    color: path.color,
                    bounds: if transformed {
                        matrix.transform_scaled_bounds(path.clipped_bounds())
                    } else {
                        path.clipped_bounds()
                    },
                }
    ...
    fn draw_paths_from_intermediate(
        ...
        let sprites = if paths.last().unwrap().order == first_path.order {
            paths
                .iter()
                .map(|path| {
                    let bounds = if path.spatial_id.is_identity() {
                        path.clipped_bounds()
                    } else {
                        scene
                            .spatial_matrix(path.spatial_id)
                            .transform_scaled_bounds(path.clipped_bounds())
                    };
                    PathSprite { bounds }
                })
                .collect::<Vec<_>>()
        } else {
            let mut bounds = if first_path.spatial_id.is_identity() {
                first_path.clipped_bounds()
            } else {
                scene
                    .spatial_matrix(first_path.spatial_id)
                    .transform_scaled_bounds(first_path.clipped_bounds())
            };
            for path in paths.iter().skip(1) {
                let path_bounds = if path.spatial_id.is_identity() {
                    path.clipped_bounds()
                } else {
                    scene
                        .spatial_matrix(path.spatial_id)
    ...
    fn draw_underlines(&mut self, start: usize, len: usize, transformed: bool) -> Result<()> {
        if len == 0 {
            return Ok(());
        }
        let devices = self.devices.as_ref().context("devices missing")?;
        ...
        if transformed {
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed underlines without uploaded spatial state");
            };
            self.pipelines.underline_pipeline.draw_range_transformed(
                &devices.device_context,
                batch_params,
                states,
                clips,
    ...
    fn draw_monochrome_sprites(
        ...
        if transformed {
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed monochrome sprites without uploaded spatial state");
            };
            self.pipelines
                .mono_sprites
                .draw_range_with_texture_transformed(
                    &devices.device_context,
                    &texture_view,
                    batch_params,
                    sampler,
    ...
    fn draw_subpixel_sprites(
        ...
        if transformed {
            // Core disables LCD text under transforms; if a transformed
            // subpixel batch still arrives, draw it with the monochrome
            // transformed shaders (identical instance layout).
            let devices = self.devices.as_ref().context("devices missing")?;
            let texture_view = self.atlas.get_texture_view(texture_id);
            let batch_params = self
                .globals
                ...
                .context("batch params buffer missing")?;
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed subpixel sprites without uploaded spatial state");
            };
            return self
                .pipelines
                .subpixel_sprites
    ...
    fn draw_polychrome_sprites(
        ...
        if transformed {
            let Some((states, clips)) = self.spatial_views() else {
                anyhow::bail!("transformed polychrome sprites without uploaded spatial state");
            };
            self.pipelines
                .poly_sprites
                .draw_range_with_texture_transformed(
...
impl DirectXRenderPipelines {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        let shadow_pipeline = PipelineState::new_with_transformed(
            device,
            "shadow_pipeline",
            ShaderModule::Shadow,
            Some(ShaderModule::ShadowTransformed),
            4,
            create_blend_state(device)?,
        )?;
        let quad_pipeline = PipelineState::new_with_transformed(
            device,
            "quad_pipeline",
            ShaderModule::Quad,
            Some(ShaderModule::QuadTransformed),
            64,
            create_blend_state(device)?,
        )?;
        ...
        let underline_pipeline = PipelineState::new_with_transformed(
            device,
            "underline_pipeline",
            ShaderModule::Underline,
            Some(ShaderModule::UnderlineTransformed),
            4,
            create_blend_state(device)?,
        )?;
        let mono_sprites = PipelineState::new_with_transformed(
            device,
            "monochrome_sprite_pipeline",
            ShaderModule::MonochromeSprite,
            Some(ShaderModule::MonochromeSpriteTransformed),
            512,
            create_blend_state(device)?,
        )?;
        // Subpixel text never carries a spatial transform (core falls back to
        // the grayscale atlas under transforms). The transformed shaders are
        // still wired so an unexpected transformed batch renders instead of
        // erroring; the layouts are identical to monochrome sprites.
        let subpixel_sprites = PipelineState::new_with_transformed(
            device,
        ...
        let poly_sprites = PipelineState::new_with_transformed(
            device,
            "polychrome_sprite_pipeline",
            ShaderModule::PolychromeSprite,
            Some(ShaderModule::PolychromeSpriteTransformed),
            16,
            create_blend_state(device)?,
        )?;
...
struct PipelineState<T> {
    ...
    transformed_vertex: Option<ID3D11VertexShader>,
    transformed_fragment: Option<ID3D11PixelShader>,
    buffer: ID3D11Buffer,
    buffer_size: usize,
...
impl<T> PipelineState<T> {
    fn new(
        ...
        Self::new_with_transformed(device, label, shader_module, None, buffer_size, blend_state)
    }

    fn new_with_transformed(
        device: &ID3D11Device,
        label: &'static str,
        shader_module: ShaderModule,
        transformed_module: Option<ShaderModule>,
        buffer_size: usize,
        blend_state: ID3D11BlendState,
    ) -> Result<Self> {
        let vertex = {
        ...
        let (transformed_vertex, transformed_fragment) = match transformed_module {
            Some(module) => {
                let vertex = {
                    let raw_shader = RawShaderBytes::new(module, ShaderTarget::Vertex)?;
                    create_vertex_shader(device, raw_shader.as_bytes())?
                };
                let fragment = {
                    let raw_shader = RawShaderBytes::new(module, ShaderTarget::Fragment)?;
                    create_fragment_shader(device, raw_shader.as_bytes())?
                };
    ...
    fn draw_range_transformed(
        &self,
        device_context: &ID3D11DeviceContext,
        batch_params_buffer: &ID3D11Buffer,
        spatial_states: &[Option<ID3D11ShaderResourceView>],
        spatial_clips: &[Option<ID3D11ShaderResourceView>],
        first_instance: u32,
        instance_count: u32,
    ) -> Result<()> {
        let (vertex, fragment) = match (
            self.transformed_vertex.as_ref(),
            self.transformed_fragment.as_ref(),
        ) {
            (Some(vertex), Some(fragment)) => (vertex, fragment),
            _ => anyhow::bail!("{} pipeline has no transformed shaders", self.label),
        };
        anyhow::ensure!(
            first_instance as usize + instance_count as usize <= self.buffer_size,
            "DirectX instance range exceeds the {} buffer",
        ...
        update_batch_start(device_context, batch_params_buffer, first_instance)?;
        set_transformed_pipeline_state(
            device_context,
            slice::from_ref(&self.view),
            spatial_states,
            spatial_clips,
    ...
    fn draw_range_with_texture_transformed(
        &self,
        device_context: &ID3D11DeviceContext,
        texture: &[Option<ID3D11ShaderResourceView>],
        batch_params_buffer: &ID3D11Buffer,
...
#[inline]
fn ensure_spatial_buffer<T>(
    device: &ID3D11Device,
    buffer: &mut Option<ID3D11Buffer>,
    view: &mut Option<ID3D11ShaderResourceView>,
    capacity: &mut usize,
    data: &[T],
) -> Result<()> {
    if *capacity < data.len() {
        ...
        anyhow::ensure!(
            required_size <= MAX_INSTANCE_BUFFER_SIZE,
            "spatial buffer needs {required_size} bytes, above the maximum of {MAX_INSTANCE_BUFFER_SIZE}"
        );
        let new_capacity = data
            .len()
            .next_power_of_two()
            .min(MAX_INSTANCE_BUFFER_SIZE / element_size);
        let new_buffer = create_buffer(device, element_size, new_capacity)?;
        let new_view = create_buffer_view(device, &new_buffer)?;
...
/// Apply a device-pixel affine matrix to a scaled-pixel point.
fn apply_scaled_matrix(
...
#[inline]
fn set_transformed_pipeline_state(
    device_context: &ID3D11DeviceContext,
    buffer_view: &[Option<ID3D11ShaderResourceView>],
    spatial_states: &[Option<ID3D11ShaderResourceView>],
    spatial_clips: &[Option<ID3D11ShaderResourceView>],
    ...
    unsafe {
        device_context.VSSetShaderResources(1, Some(buffer_view));
        device_context.PSSetShaderResources(1, Some(buffer_view));
        device_context.VSSetShaderResources(2, Some(spatial_states));
        device_context.PSSetShaderResources(2, Some(spatial_states));
        device_context.VSSetShaderResources(3, Some(spatial_clips));
        device_context.PSSetShaderResources(3, Some(spatial_clips));
        device_context.IASetPrimitiveTopology(topology);
        device_context.VSSetShader(vertex_shader, None);
        device_context.PSSetShader(fragment_shader, None);
        device_context.OMSetBlendState(blend_state, None, 0xFFFFFFFF);
...
pub(crate) mod shader_resources {
    ...
    pub(crate) enum ShaderModule {
        ...
        ShadowTransformed,
        Underline,
        UnderlineTransformed,
        PathRasterization,
        PathSprite,
        MonochromeSprite,
        MonochromeSpriteTransformed,
        SubpixelSprite,
        PolychromeSprite,
        PolychromeSpriteTransformed,
        EmojiRasterization,
    }
    ...
    impl<'t> RawShaderBytes<'t> {
        ...
        fn from_bytes(module: ShaderModule, target: ShaderTarget) -> Self {
            let bytes = match module {
                ...
                ShaderModule::QuadTransformed => match target {
                    ShaderTarget::Vertex => QUAD_TRANSFORMED_VERTEX_BYTES,
                    ShaderTarget::Fragment => QUAD_TRANSFORMED_FRAGMENT_BYTES,
                },
                ShaderModule::Shadow => match target {
                    ShaderTarget::Vertex => SHADOW_VERTEX_BYTES,
                    ShaderTarget::Fragment => SHADOW_FRAGMENT_BYTES,
                },
                ShaderModule::ShadowTransformed => match target {
                    ShaderTarget::Vertex => SHADOW_TRANSFORMED_VERTEX_BYTES,
                    ShaderTarget::Fragment => SHADOW_TRANSFORMED_FRAGMENT_BYTES,
                },
                ...
                ShaderModule::UnderlineTransformed => match target {
                    ShaderTarget::Vertex => UNDERLINE_TRANSFORMED_VERTEX_BYTES,
                    ShaderTarget::Fragment => UNDERLINE_TRANSFORMED_FRAGMENT_BYTES,
                },
                ...
                ShaderModule::MonochromeSpriteTransformed => match target {
                    ShaderTarget::Vertex => MONOCHROME_SPRITE_TRANSFORMED_VERTEX_BYTES,
                    ShaderTarget::Fragment => MONOCHROME_SPRITE_TRANSFORMED_FRAGMENT_BYTES,
                },
                ...
                ShaderModule::PolychromeSpriteTransformed => match target {
                    ShaderTarget::Vertex => POLYCHROME_SPRITE_TRANSFORMED_VERTEX_BYTES,
                    ShaderTarget::Fragment => POLYCHROME_SPRITE_TRANSFORMED_FRAGMENT_BYTES,
                },
    ...
    impl ShaderModule {
        pub fn as_str(self) -> &'static str {
            match self {
                ...
                ShaderModule::QuadTransformed => "quad_transformed",
                ShaderModule::Shadow => "shadow",
                ShaderModule::ShadowTransformed => "shadow_transformed",
                ShaderModule::Underline => "underline",
                ShaderModule::UnderlineTransformed => "underline_transformed",
                ShaderModule::PathRasterization => "path_rasterization",
                ShaderModule::PathSprite => "path_sprite",
                ShaderModule::MonochromeSprite => "monochrome_sprite",
                ShaderModule::MonochromeSpriteTransformed => "monochrome_sprite_transformed",
                ShaderModule::SubpixelSprite => "subpixel_sprite",
                ShaderModule::PolychromeSprite => "polychrome_sprite",
                ShaderModule::PolychromeSpriteTransformed => "polychrome_sprite_transformed",
