...
impl MacPlatform {
    ...
    /// Creates a platform whose AppKit loop is driven by a foreign main loop.
    ...
    /// Processes only AppKit work that is ready now and returns immediately.
    ...
    unsafe fn register_with_appkit(&self, app: id, delegate: id) {
        unsafe {
            let self_ptr = self as *const Self as *mut c_void;
            (*app).set_ivar(MAC_PLATFORM_IVAR, self_ptr);
            (*delegate).set_ivar(MAC_PLATFORM_IVAR, self_ptr);
            REGISTERED_MAC_PLATFORM.store(self_ptr, Ordering::Release);
        }
    }
    ...
    unsafe fn clear_appkit_registration(&self) {
        unsafe {
            ...
            if *(*app).get_ivar::<*mut c_void>(MAC_PLATFORM_IVAR) == self_ptr {
                (*app).set_ivar(MAC_PLATFORM_IVAR, null_mut::<c_void>());
            }

            let delegate = NSWindow::delegate(app);
            if delegate == nil
                || *(*delegate).get_ivar::<*mut c_void>(MAC_PLATFORM_IVAR) != self_ptr
            {
                return;
            }
...
fn registered_mac_platform(context: *mut c_void) -> Option<&'static MacPlatform> {
...
unsafe fn get_mac_platform(object: &mut Object) -> Option<&MacPlatform> {
    unsafe {
        ...
        (!platform_ptr.is_null()).then(|| &*(platform_ptr as *const MacPlatform))
    }
}
